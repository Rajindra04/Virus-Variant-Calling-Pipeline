...
def plot_coverage(coverage_file, output_dir):
    ...
    print(f"Coverage plot generated: {coverage_plot_file}")
...
def add_read_groups(bam_file, sample_name, output_dir):
    ...
    print(f"Add read groups output: {stdout}")
    print(f"Add read groups error: {stderr}")
...
def run_variant_calling(bam_file, reference_fasta, sample_name, output_dir):
    ...
    print(f"GATK HaplotypeCaller output: {stdout}")
    print(f"GATK HaplotypeCaller error: {stderr}")
    return raw_vcf
...
def run_snpeff_annotation(raw_vcf, sample_name, output_dir, reference_name="denv1"):
    ...
    print(f"SnpEff annotation output: {stdout}")
    print(f"SnpEff annotation error: {stderr}")
...
def run_snpsift_extract(annotated_vcf, sample_name, output_dir):
    ...
    print(f"SnpSift extract output: {stdout}")
    print(f"SnpSift extract error: {stderr}")
...
if __name__ == '__main__':
    ...
    for bam_file in bam_files:
        ...
        try:
            ...
            print(f"Consensus sequence, coverage file, coverage plot, annotated VCF, SnpEff reports, and SnpSift output generated: {os.path.join(output_dir, consensus_fasta)}, {coverage_file}, {annotated_vcf}, {summary_html}, {summary_csv}, {summary_txt}, {snpsift_output}")
